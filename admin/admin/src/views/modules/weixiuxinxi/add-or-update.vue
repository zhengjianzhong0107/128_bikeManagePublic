<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="单车编号" prop="danchebianhao">
          <el-select  @change="danchebianhaoChange" v-model="ruleForm.danchebianhao" placeholder="请选择单车编号">
            <el-option
                v-for="(item,index) in danchebianhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.danchebianhao" label="单车编号" prop="danchebianhao">
              <el-input v-model="ruleForm.danchebianhao" 
                placeholder="单车编号" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="维修地址" prop="weixiudizhi">
          <el-input v-model="ruleForm.weixiudizhi" 
              placeholder="维修地址" clearable  :readonly="ro.weixiudizhi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="维修地址" prop="weixiudizhi">
              <el-input v-model="ruleForm.weixiudizhi" 
                placeholder="维修地址" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.weixiubianhao = true;
          ...
          if(o=='danchebianhao'){
            this.ruleForm.danchebianhao = obj[o];
	    this.ro.danchebianhao = true;
            continue;
          }
      ...
      this.$http({
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/gongxiangdanche/danchebianhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.danchebianhaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
            this.zhuangtaiOptions = "待维修,已维修".split(',')
    },
    // 下二随
    danchebianhaoChange () {
      this.$http({
        url: `follow/gongxiangdanche/danchebianhao?columnValue=`+ this.ruleForm.danchebianhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.guzhangmiaoshu = this.ruleForm.guzhangmiaoshu.replace(reg,'../../../springbootptg21/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
